...
import { checkoutShipment, extractMeConfig } from '@/lib/melhor-envio'
...
export async function POST(request: Request) {
  try {
    ...
    if (!order?.melhor_envio_shipment_id) {
      return NextResponse.json({ error: 'Shipment not in cart yet' }, { status: 400 })
    }
    ...
    console.error('ME checkout error:', error)
    return NextResponse.json({ error: error.message || 'Error checking out' }, { status: 500 })
  }
}
